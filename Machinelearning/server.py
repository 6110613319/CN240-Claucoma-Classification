...
def exudate_img(image):
    ...
    cv2.imwrite('./keeptest/OtherGreenE.jpg',g)
    gray_scale = cv2.cvtColor(fundus_dash,cv2.COLOR_BGR2GRAY)
    cv2.imwrite('./keeptest/OtherGrayE.jpg',gray_scale)
    ...
    cv2.imwrite('./keeptest/OtherStru1.jpg', bv_image)
...
def muteluh_model(img):
    cdr = segment(img)
    exudate = exudate_img(img)
    XTest = [[cdr,exudate]]
    class_names = ['glaucoma','normal','other']
    Pkl_Filename = "DisisionTree_ML_Good.pkl"
    with open(Pkl_Filename, 'rb') as file:  
        Model = pickle.load(file)  
    Ypredict = Model.predict(XTest)
    #score = Model.predict_proba(XTest,Ypredict)
    score = Model.predict_proba(XTest)
    argmax = np.argmax(score)
    return class_names[argmax],score[0][argmax]
